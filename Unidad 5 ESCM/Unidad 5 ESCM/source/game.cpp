...
EstadoJuego estado = EstadoJuego::Waiting;




void InitGame() {
	InitWindow(screenWidth, screenHeight, "MAVI -ESCM");
	SetTargetFPS(60);
...
void UpdateGame() {
    while (!WindowShouldClose())
    {
        ...
        if (estado == EstadoJuego::Running){world.Step(2.5f / 60.0f, 8, 3);}
        ...
        Rectangle rectMeta ={metaPos.x - 25,metaPos.y - 0,50,50};
        DrawRectangleLinesEx(rectMeta, 3, RED);


        // Estados del juego

        if (estado == EstadoJuego::Waiting){DrawText("Esperando...", 20, 250, 30, DARKBLUE);}

        if (estado == EstadoJuego::Running){DrawText("Jugando", 20, 250, 30, WHITE);}

        if (estado == EstadoJuego::EventDetected){DrawText("Evento detectado!", 20, 250, 40, YELLOW);}

        if (estado == EstadoJuego::Finished){DrawText("Meta alcanzada!", 300, 355, 40, GREEN);}
       ...
       if (estado == EstadoJuego::Waiting) {DrawText("Presiona ENTER para comenzar", 250, 370, 30, DARKBLUE);}
        if (estado == EstadoJuego::Waiting){if (IsKeyPressed(KEY_ENTER))estado = EstadoJuego::Running;}        
        if (estado == EstadoJuego::EventDetected) {if (IsKeyPressed(KEY_ENTER)) { estado = EstadoJuego::Finished; }DrawText("Presiona ENTER para ganar!", 250, 370, 30, YELLOW);}
